...
def render_answer_summary(results: dict) -> None:
    mode = results.get("mode")
    ...
    if mode == "forecast":
        ...
        forecast_total = float(forecast_table["ForecastValue"].sum())
        first_month = pd.to_datetime(forecast_table.iloc[0]["Month"]).strftime("%b %Y")
        last_month = pd.to_datetime(forecast_table.iloc[-1]["Month"]).strftime("%b %Y")
        metric_label = parsed_query["metric"].replace("_", " ").title()
        st.success(
            f"Forecasted {metric_label} for {first_month} to {last_month}: "
            f"{forecast_total:,.2f}."
        )
        st.caption(f"Filters used: {format_filters(parsed_query['filters'])}")
        return

    if mode == "analysis":
        analysis = results["analysis_result"]
        top_rows = analysis["top_n"]
        if not top_rows.empty:
            top_item = top_rows.iloc[0]
            st.success(
                f"Top result: {top_item[analysis['group_by']]} with "
                f"{top_item['MetricValue']:,.2f}."
            )
        st.caption(analysis["headline"])
        return

    if mode == "recommendation":
        top_actions = [item["Action"] for item in results["recommendations"][:3]]
        st.success("Recommended actions: " + "; ".join(top_actions) + ".")
        return

    if mode == "goal":
        summary = results["goal_plan"]["summary"]
        st.success("Goal plan generated. Review the recommended actions below.")
    ...
    if mode == "generic_forecast":
        ...
        last_month = pd.to_datetime(forecast_table.iloc[-1]["Month"]).strftime("%b %Y")
        st.success(
            f"Forecasted {results['metric_column']} for {first_month} to {last_month}: "
            f"{forecast_total:,.2f}."
        )
        return
    ...
    if mode == "generic_grouped_forecast":
        forecast_table = results["forecast_table"]
        top_group = forecast_table.iloc[0]
        st.success(
            f"Top forecast group: {top_group['Group']} with "
            f"{top_group['ForecastTotal']:,.2f} forecast total."
        )
